...
btn_get_res = ttk.Button(text="получить рекомендации", command= lambda:print(*get_recommendation(seed_objects, json_data, weights_attributes), sep='\n'))
btn_get_res.pack(anchor=NW, padx=6, pady=12)
...
btn_get_res = ttk.Button(text="получить рекомендации по лайкам", command= lambda:print(*get_recommendation_by_likes(likes_objects, json_data, weights_attributes), sep='\n'))
btn_get_res.pack(anchor=NW, padx=200, pady=12)
#для дизлайков----------------------------
btn_add_likes = ttk.Button(text="добавить массив дизлайков", command=lambda:add_dislikes_object(dislike_objects, json_data, entry.get()))
btn_add_likes.pack(anchor=NW, padx=400, pady=6)

btn_get_res = ttk.Button(text="получить рекомендации \n по лайкам и дизлайкам", command= lambda:print(*get_recommendation_by_likes_and_dislike(likes_objects, dislike_objects, json_data, weights_attributes), sep='\n'))
btn_get_res.pack(anchor=NW, padx=400, pady=12)
